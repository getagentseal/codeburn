...
| **OpenClaw** | `~/.openclaw/agents/<agent>/agent/openclaw-agent.sqlite` (per-agent SQLite store, since the 2026-09-01 migration); legacy `.jsonl` under `~/.openclaw` / `.clawdbot` / `.moltbot` / `.moldbot` | Token usage comes from assistant message `usage` blocks; the model from `modelId` or `message.model`. The SQLite store is read in batches; schema-23 zstd rows need Node 22.15+. |
