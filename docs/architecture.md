...
- the provider's own endpoint field, when it does not. `routeFromProviderField(value)` maps Hermes' `billing_provider` and OpenCode's `providerID`: `bedrock` and the exact literal `openrouter` become registered routes; direct doors (`anthropic`, `openai`, …) map to nothing, because the unsuffixed row *is* the direct row. Only doors with usage-bearing sessions on disk are registered, the same rule the id shapes follow.
