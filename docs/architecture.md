...
- the provider's own endpoint column, when it does not. `routeFromProviderField(value)` maps Hermes' `billing_provider` values `bedrock` and the exact literal `openrouter`; direct doors (`anthropic`, `openai`, …) map to nothing, because the unsuffixed row *is* the direct row. Only doors with usage-bearing sessions on disk are registered, the same rule the id shapes follow.
