...
### Fixed
- **`codeburn yield` stops calling work abandoned that shipped through a merge the session's time window missed.** A session was labeled abandoned when no commit credited to it landed on main, but commits are credited by timestamp window, so a feature branch merged after the session ended — or merged as a GitHub squash, which lands the work under a brand-new SHA the window can never see — left fully shipped sessions reading as dead. When the session logs record the git branch it ran on (Claude Code captures this per turn), yield now checks whether that branch demonstrably reached main, whenever that happened: by ancestry for true and rebase merges (everything at or below the branch ref is on main), and by tree equality for squash merges (the squashed commit's tree is byte-identical to the branch tip; that rule requires the branch to carry commits of its own and a tip different from its base, so an untouched or self-reverted branch cannot match). Abandoned and ambiguous sessions rescue to productive; reverted keeps its verdict, which already describes shipped-then-undone work. Sessions without branch metadata (most non-Claude providers) are unchanged. Fixes #1442.
