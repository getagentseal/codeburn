...
- **`codeburn status --format menubar-json` persists the base payload for the default optimize path and re-derives only the optimize block on a snapshot hit.** The same per-query snapshot now covers polls that carry `optimize` findings (the default; the desktop app's `buildOverviewArgs` has always sent the menubar-json query without `--no-optimize`), serving the heavy parse and aggregation from disk and re-running only `scanAndDetect` over the unchanged corpus to pick up `~/.claude` and project-level state the corpus fingerprint never observes (`settings.json`, `CLAUDE.md`, defined skills, agents, commands, MCP config). The persisted record carries the base payload with an empty `optimize` placeholder, so a mutation in those non-fingerprinted inputs between two requests at the same fingerprint is reflected in the second response even though the rest of the payload is byte-identical to the first. The save gate from #999 is preserved verbatim (`stale !== true`, no `hydration`, complete hydration), the queryKey already partitions `optimize: true` from `optimize: false` snapshots, and the resident `codeburn serve` in-memory output memo (keyed on the full argv) already gives `optimize=true` and `--no-optimize` distinct entries. The desktop app sees the new hit rate with zero changes of its own. (#1135)
