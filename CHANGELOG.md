...
### Added
- **A model billed through AWS Bedrock is reported as its own row, separate from the same model billed direct.** Claude Code with `CLAUDE_CODE_USE_BEDROCK=1`, Hermes on its `bedrock` provider, and any other tool that keeps the vendor's id record Bedrock's spelling — `anthropic.claude-haiku-4-5-20251001-v1:0`, `openai.gpt-5.6-luna`, with or without a `us.`/`eu.`/`global.` inference-profile prefix, an `arn:aws:bedrock:…` wrapper or LiteLLM's `bedrock/` prefix. Reports key model rows by display name, and those ids either surfaced raw or, had they been aliased to the friendly name, would have merged with the direct-API row and hidden which invoice the spend landed on. `getModelRoute` now recognises the Bedrock shapes and `getShortModelName` renders them as `Haiku 4.5 (Bedrock)` / `GPT-5.6 Luna (Bedrock)`: every Bedrock spelling of one model shares a row, the direct row is unchanged, and pricing still runs on the full id so a cross-region profile keeps its own rate. The `-v1:0` version suffix is also no longer mistaken for an Ollama tag, so an unpriced Bedrock model shows up in Unpriced instead of being treated as free local inference. Bedrock is the first route; OpenRouter, Vertex and Azure spellings slot into the same hook.
