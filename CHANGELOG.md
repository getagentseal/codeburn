...
### Added (CLI)
- **Counterfactual pricing.** `codeburn compare --reprice <model>` now
  recalculates recorded calls against a target model's pricing, with text and
  JSON output showing actual spend, what-if spend, savings, project impact,
  source-model impact, and top affected sessions.
