...
### Fixed
 - **Ranged commands decode cache shards incrementally instead of assembling whole multi-hundred-megabyte files.** Session month shards now decode one turn at a time and keep only in-range turns (with the exact same kept/dropped/carry contract as before, verified by the range-filter suite, including a 30k-turn record test); the codex result cache streams instead of `JSON.parse`-ing whole, serves discovery labels from calls-free metadata, and publishes writes by merge instead of full rewrite; shard loads run serially and the pre-lock snapshot is released before the canonical reload instead of overlapping it. All retained strings are detached from tokenizer buffers (sliced views pinned whole input chunks: ~1.2GB unexplained heap on this corpus), dropped dedup keys are 128-bit digests, and provider-scoped queries skip unrelated sections. This bounds cache-I/O transients; corpora with a very large same-day slice can still exceed small heaps.
