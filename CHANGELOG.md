...
### Changed
- **The Flatpak package no longer gets read-write access to your whole home directory.** The manifest asked for `--filesystem=home`; it now asks for `home:ro`, with two declared writable exceptions (`~/.config/codeburn` for settings and the act journal, `xdg-download` so `codeburn export` has a real destination). Reading stays unrestricted, which matters because CodeBurn reads both provider dot-directories and the user's own project directories for git attribution. The launcher also keeps the cache in Flatpak's persistent XDG cache instead of the sandbox tmpfs, and points OpenCode back at its host store, neither overriding a value the user already set. (#1011)
