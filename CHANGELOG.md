...
### Fixed
- **Bundled pricing is refreshed, and a refresh can no longer leave a model without pricing.** 5,938 primary entries: 1,210 added and 298 repriced against the 0.9.24 bundle (among the additions, upstream shipped the Grok Build family, which makes `grok-4.6-build` priceable so the Grok parser's authoritative-model rule now prefers the real modelUsage id). Four entries are gone upstream (the two friendliai llama-3.1 rows, bare and prefixed) and seven more are newly excluded by the bundler's per-token-output filter (twelvelabs embedding and gpt-image-2 rows that publish no per-token output cost) — a filter rule change, not upstream deletions. Two bundler rules harden the refresh itself: a vendor-prefixed alias with richer published rates (cache read/write) can no longer displace the publisher's entry for the same model under first-write-wins (a new `nebius/MiniMaxAI/MiniMax-M3` without cache-read rates was silently replacing the MiniMax entry that carries them, repricing 1,574 calls by −$2.99 through the `input × 0.1` fallback), and previously-priced fallback entries carry forward when neither the new primary nor the gap-fill covers their key under the exact or vendor-prefixed form the resolution pipeline actually queries — nine models (qwen3.5-plus, grok-latest, gpt-latest, mercury-2.5-preview, inkling-small:batch, nex-n2-mini/pro, gemma-4-31b-it:batch, gpt-oss-20b:batch) would otherwise have lost all pricing; the fallback moves from 203 to 60 entries.
