...
### Fixed (desktop)
- **The desktop app stops re-scanning the filesystem for the CLI on every request.** Each of the app's JSON reads (every section poll, every prefetch warm) resolved the `codeburn` binary from scratch: a stat sweep over every PATH entry plus a readdir of the nvm versions tree, repeated per request, purely to return the same path. Resolution and the derived spawn PATH are now memoized against the full set of inputs that can change the answer (`CODEBURN_BIN`, the bundled-CLI path, the dev-server and persisted-path overrides, `CODEBURN_PATH_DIRS`, `PATH` and `NVM_DIR`), so an unchanged environment costs a string compare; a changed variable — or a test re-pointing an override — recomputes from scratch. A miss is deliberately never cached, so a CLI installed, or a path persisted, while the app is open is still discovered on the next read.
