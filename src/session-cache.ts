...
export type RangeFilteredMeta = {
  ...
  /// sides uniformly, so membership is exact up to 128-bit collision.
  droppedKeys: string[]
  ...
  /// prefix in order (mirrors the serve loops' own carry initialization).
...
/// 128-bit digest of a dedup identity for memory-compact comparison. Raw
/// dropped keys dominate retained heap at corpus scale (~140MB of characters
/// for ~1.1M keys here), so markers and shared sets store digests instead.
/// Every insertion and lookup in the pipeline hashes uniformly (see DedupSet),
/// so membership is exact up to 2^-128 collision — far below any operational
/// risk, and a collision would suppress (not fabricate) a single call.
/// Implemented inline (cyrb-style 32-bit mix, dual-seeded for 128 bits)
/// rather than via createHash: ~1M keys are hashed per full load, where hash
/// objects would add seconds of CPU and hundreds of MB of churn against the
/// very bound this exists to protect. Hex32 keeps Set<string> infrastructure
...
/// A Set<string> that stores 128-bit digests (see digestDedupKey) instead of
/// raw dedup identities. Drop-in for the shared dedup sets: add/has/delete
/// digest transparently, so producers keep passing raw keys and iteration
/// yields digests. Marker seeding uses addHashed (markers already hold
/// digests). Consumers that match by key PREFIX (not exact membership) cannot
/// use this set — they keep exact keys via the conversations channel below.
export class DedupSet extends Set<string> {
  override add(key: string): this {
    super.add(digestDedupKey(key))
    ...
    if (bare !== null) super.add(digestDedupKey(bare))
    return this
  }
  override has(key: string): boolean {
    return super.has(digestDedupKey(key))
  }
  override delete(key: string): boolean {
    return super.delete(digestDedupKey(key))
  }
  ...
  /// Build from already-digested values without re-hashing (cloning a set).
  static fromDigests(digests: Iterable<string>): DedupSet {
    const set = new DedupSet()
    for (const digest of digests) set.addHashed(digest)
    return set
  }
}
...
function sliceItemFor(turn: CachedTurn, keep: boolean): SliceItem {
 ...
 /// wrappers plus key/conversation arrays retained to file end; the
 /// accumulator keeps only what the marker and output need — kept turns,
 /// dropped digests, kept digests, counters, and scalar carry/suffix state.
 /// Decision logic lives in finishSlice (shared with projectTurnSlice), so
 /// the second-pass fallback in filterShardFile behaves identically.
 ...
 /// Fold one live turn with zero per-turn allocation beyond the payload
 /// digests and bare keys themselves: digests stream straight into the shared
 /// arrays, scalars update counters. Mirrors sliceItemFor plus the carry and
 /// suffix-branch scans positionally (last-wins carry, suffix reset on kept).
 ...
 /// scan's whole-list coverage via anyBranch. No string work happens here:
 /// every turn reaching the accumulator was assembled by assembleTokens,
 /// which already detached all string tokens (see shard-stream.ts), and bare
 /// conversation keys are fresh template strings (see
 /// rpcConversationBareKey) — everything retains directly.
